  ...
  for (const cookieName in cookie) {
    switch (cookieName) {
      case "LockedOutMessage": {
        const lockedOutMessage = cookie["LockedOutMessage"];

        return lockedOutError(lockedOutMessage, context);
      }
      case "EmailClaimError": {
        const emailClaimErrorMsg = cookie["EmailClaimError"];

        return emailClaimError(emailClaimErrorMsg);
      }
      case "CreateOrLinkError": {
        const createOrLinkErrorMsg = cookie["CreateOrLinkError"];

        return createOrLinkError(createOrLinkErrorMsg);
      }
      case "ExternalSigninError": {
        const externalSigninErrorMsg = cookie["ExternalSigninError"];

        console.log("external signin error: " + externalSigninErrorMsg);

        return externalSigninError(externalSigninErrorMsg);
      }
      case "MessageCookie": {
        const successCookieMsg = cookie["MessageCookie"];
